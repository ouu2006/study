...
import type { Lv5Message } from '../types'
import s from './Lv5ChatDetailPage.module.css'
...
export default function Lv5ChatDetailPage({ chatId, chatName, messages, onSend, onBack, embedded }: Props) {
    ...
    <div className={`page page-enter ${s.root}`}>
      {embedded ? (
        <div className={s.embeddedHeader}>
          <div className={s.embeddedTitle}>{chatName}</div>
          <div className={s.embeddedMenu}>
        ...
        <div className={s.header}>
          <div className={s.headerBack} onClick={onBack}>
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
              <polyline points="15 18 9 12 15 6" />
            </svg>
          </div>
          <div className={s.headerTitle}>{chatName}</div>
...
function MessageRow({ message, onImageClick }: { message: Lv5Message; onImageClick: (bg: string) => void }) {
      ...
      <div className={`${s.bubbleCol} ${isSelf ? s.bubbleColSelf : s.bubbleColOther}`}>
        ...
        {message.kind === 'file' && (
          <div className={`${s.fileCard} ${!isSelf ? s.fileCardOther : ''}`}>
            <div className={s.fileIcon} style={{ background: message.fileIconBg ?? '#2B5797' }}>
              <span>{message.fileIcon ?? 'W'}</span>
            </div>
...
function groupByTime(messages: Lv5Message[]) {
  const out: { time: string; messages: Lv5Message[] }[] = []
