...
import Lv3ChatListPage from './pages/Lv3ChatListPage'
import Lv3ChatDetailPage from './pages/Lv3ChatDetailPage'
import Lv3DiscoverPage from './pages/Lv3DiscoverPage'
import Lv3ContactsPage from './pages/Lv3ContactsPage'
import Lv3ProfilePage from './pages/Lv3ProfilePage'
import { useLv3Chats } from './hooks/useLv3Chats'
import { useBreakpoint } from './hooks/useMediaQuery'
import s from './Lv3App.module.css'

export default function Lv3App() {
  const chatState = useLv3Chats()
  const { isDesktop } = useBreakpoint()

  if (isDesktop) {
    return <Lv3Desktop chatState={chatState} />
  }
  return <Lv3Mobile chatState={chatState} />
}

/* ============ 移动端：手机框 + 单栏路由 ============ */
function Lv3Mobile({ chatState }: { chatState: ReturnType<typeof useLv3Chats> }) {
  const navigate = useNavigate()

  const openChat = useCallback(
    (id: string) => {
      chatState.markRead(id)
      navigate(`/lv3/chat/${id}`)
    },
    [chatState, navigate],
  )
        ...
        <Route
          ...
          element={
            <Lv3ChatListPage
              ...
              onOpen={openChat}
              onDelete={chatState.deleteChat}
              onTogglePin={chatState.togglePin}
              onToggleMute={chatState.toggleMute}
              onToggleRead={chatState.toggleRead}
            />
          }
        />
        <Route
          path="/chat/:id"
          element={<Lv3MobileChatDetail chatState={chatState} />}
        />
        <Route path="/contacts" element={<Lv3ContactsPage />} />
        <Route path="/discover" element={<Lv3DiscoverPage />} />
        <Route path="/profile" element={<Lv3ProfilePage onReset={chatState.resetChats} />} />
      </Routes>
    </PhoneFrame>
  )
}

function Lv3MobileChatDetail({ chatState }: { chatState: ReturnType<typeof useLv3Chats> }) {
  const { id = '' } = useParams()
  const navigate = useNavigate()
...
function Lv3Desktop({ chatState }: { chatState: ReturnType<typeof useLv3Chats> }) {
  const { pathname } = useLocation()
  const navigate = useNavigate()

  const activeTab: TabKey = pathname.startsWith('/lv3/contacts')
    ? 'contacts'
    : pathname.startsWith('/lv3/discover')
      ? 'discover'
      : pathname.startsWith('/lv3/profile')
        ? 'profile'
        : 'chats'

  const selectedChatId = pathname.startsWith('/lv3/chat/') ? pathname.slice('/lv3/chat/'.length) : ''
  ...
  const goTab = (tab: TabKey) => {
    if (tab === 'chats') navigate('/lv3')
    else navigate(`/lv3/${tab}`)
  }
  ...
  return (
    <div className={s.desktopShell}>
      ...
      {activeTab !== 'chats' && (
        <div className={s.mainCol}>
          {activeTab === 'contacts' && <Lv3ContactsPage />}
          {activeTab === 'discover' && <Lv3DiscoverPage />}
          {activeTab === 'profile' && <Lv3ProfilePage onReset={chatState.resetChats} />}
        </div>
      )}
    </div>
  )
