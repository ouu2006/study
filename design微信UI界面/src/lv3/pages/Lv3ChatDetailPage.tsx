...
import type { Lv3Message } from '../types'
import s from './Lv3ChatDetailPage.module.css'
...
export default function Lv3ChatDetailPage({ chatId, chatName, messages, onSend, onBack, embedded }: Props) {
  ...
  return (
    <div className={`page page-enter ${s.root}`}>
      ...
      <div className={s.chatArea} ref={scrollRef}>
        {messages.length === 0 && (
          <div style={{ textAlign: 'center', color: 'var(--wechat-text-tertiary)', fontSize: 13, marginTop: 40 }}>
            暂无消息，发送第一条消息开始聊天
          </div>
        )}
        {groups.map((group) => (
...
function MessageRow({ message, onImageClick }: { message: Lv3Message; onImageClick: (bg: string) => void }) {
...
function groupByTime(messages: Lv3Message[]) {
  const out: { time: string; messages: Lv3Message[] }[] = []
