...
import s from './Lv4ProfilePage.module.css'
...
export default function Lv4ProfilePage({ onReset }: { onReset?: () => void }) {
    ...
    <div className="page">
      ...
      <div className={`scroll-area ${s.content}`}>
        ...
        <div className={s.levelCard} onClick={() => navigate('/home')}>
          <span className={s.levelBadge}>Lv.4 全端体验</span>
          <div className={s.levelTitle}>微信UI界面 · 全端体验版</div>
          <div className={s.levelDesc}>
            基于设计稿 5 页面高保真还原：聊天列表 / 文件传输助手 / 通讯录 / 发现 / 我。
            四 Tab 全联动 + 跨页导航 + 联系人详情 + 消息发送持久化 + 桌面端分栏多端适配。
          </div>
          <div className={s.levelLink}>
            <svg width="12" height="12" viewBox="0 0 16 16" fill="none">
              <path d="M10 4l-4 4 4 4" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
            查看完整学习路线图（Lv.1 → Lv.2 → Lv.3 → Lv.4）
          </div>
        </div>
          ...
          {SERVICE_ROWS.map((r, i) => (
            <button
              ...
              onClick={() => {
                ...
                else if (r.action === 'nav-lv3') navigate('/lv3')
